    ...
    <div class="w-full h-[90%]">
      ...
      <ElsDialog v-model="addDialogVisible" title="添加">
        <el-form ref="addFormInstance" :model="addForm" :rules="addFormRule" label-width="100">
          ...
          <el-form-item label="标签描述" prop="description">
            <el-input v-model="addForm.description" placeholder="请输入标签描述" />
          </el-form-item>
        </el-form>
        <template #footer>
      ...
      <ElsDialog v-model="editDialogVisible" title="编辑">
        <el-form ref="editFormInstance" :model="editForm" :rules="editFormRule" label-width="100">
          ...
          <el-form-item label="标签描述" prop="description">
            <el-input v-model="editForm.description" placeholder="请输入标签描述" />
          </el-form-item>
        </el-form>
        <template #footer>
...
import type { ArticleTag, ArticleTagList } from '@/types';
...
const getTableData = async (): Promise<void> => {
  const { data } = await fetchArticleTagList(currentPage.value, pageSize.value, keyword.value);
...
const handleView = (row: ArticleTag): void => {
  viewForm.value = { ...row };
...
const handleAdd = async (formEl: FormInstance | undefined): Promise<void> => {
  ...
  await formEl.validate(async (valid: boolean) => {
    if (valid) {
      const { data } = await addArticleTagApi({
        ...addForm.value
      });
      if (data.code === 201) {
...
// 删除
const handleDelete = (row: ArticleTag | ArticleTagList): void => {
  ElMessageBox.confirm('删除后无法恢复，是否确认删除？', '警告', {
    ...
    if (row instanceof Array) {
      ...
      // 批量删除
      result = await deleteArticleTagApi(row.map((item) => item.id as number));
    } else {
      // 单个删除
      result = await deleteArticleTagApi(row.id as number);
    }
...
const handleEdit = async (row: ArticleTag): Promise<void> => {
...
const handleConfirmEdit = async (formEl: FormInstance | undefined): Promise<void> => {
  ...
  await formEl.validate(async (valid: boolean) => {
    if (valid) {
      const { data } = await updateArticleTagApi({
        ...editForm.value,
        createdAt: undefined,
        updatedAt: undefined
      });
