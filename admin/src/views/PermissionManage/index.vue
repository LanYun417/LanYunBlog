...
<script name="PermissionManage" lang="ts" setup>
import {
...
import type { AdminPermission, AdminPermissionList } from '@/types';
...
// 表格数据
const tableData = ref<AdminPermissionList>([]);

// 表格字段
const fields: TableFields = [
  { type: 'text', label: 'ID', prop: 'id', sortable: true },
  { type: 'text', label: '权限名称', prop: 'name' },
  { type: 'text', label: '权限描述', prop: 'description' },
...
const getTableData = async (): Promise<void> => {
  const { data } = await fetchAdminPermissionList(currentPage.value, pageSize.value, keyword.value);
  total.value = data.total;
  tableData.value = data.list.map((item: AdminPermission) => {
    return {
      ...item,
...
// 选中项
const selectList = ref<AdminPermissionList>([]);
const handleSelectionChange = (items: AdminPermissionList): void => {
  selectList.value = items;
};

// 查看
const viewForm = ref<AdminPermission>({
  id: 0,
  name: '',
  description: '',
  createdAt: '',
...
const handleView = (row: AdminPermission): void => {
  viewForm.value = { ...row };
...
const addForm = ref<AdminPermission>({
  name: '',
  description: ''
});
const handleAdd = async (formEl: FormInstance | undefined): Promise<void> => {
  ...
  await formEl.validate(async (valid: boolean) => {
    if (valid) {
      const { data } = await addAdminPermissionApi({
        ...addForm.value
      });
      if (data.code === 201) {
...
// 删除
const handleDelete = (row: AdminPermission | AdminPermissionList): void => {
  ElMessageBox.confirm('删除后无法恢复，是否确认删除？', '警告', {
    ...
    if (row instanceof Array) {
      ...
      // 批量删除
      result = await deleteAdminPermissionApi(row.map((item) => item.id as number));
    } else {
      // 单个删除
      result = await deleteAdminPermissionApi(row.id as number);
    }
...
const editForm = ref<AdminPermission>({
  id: 0,
  name: '',
  description: ''
});
...
const handleEdit = async (row: AdminPermission): Promise<void> => {
  editForm.value = { ...row };
...
const handleConfirmEdit = async (formEl: FormInstance | undefined): Promise<void> => {
  ...
  await formEl.validate(async (valid: boolean) => {
    if (valid) {
      const { data } = await updateAdminPermissionApi({
        ...editForm.value,
        createdAt: undefined,
        updatedAt: undefined
      });
